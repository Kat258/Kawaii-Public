...
public class FriendCommand extends Command {
	...
	public void runCommand(String[] parameters) {
        ...
        switch (parameters[0]) {
            case "reset" -> {
                Kawaii.FRIEND.friendList.clear();
                CommandManager.sendChatMessage("§fFriends list got reset");
                return;
            }
            case "list" -> {
                if (Kawaii.FRIEND.friendList.isEmpty()) {
                ...
                boolean first = true;
                boolean start = true;
                for (String name : Kawaii.FRIEND.friendList) {
                    if (!first) {
                        friends.append(", ");
                    }
                    friends.append(name);
            ...
            case "add" -> {
                if (parameters.length == 2) {
                    Kawaii.FRIEND.addFriend(parameters[1]);
                    CommandManager.sendChatMessage("§f" + parameters[1] + (Kawaii.FRIEND.isFriend(parameters[1]) ? " §ahas been friended" : " §chas been unfriended"));
                    return;
                }
                sendUsage();
                return;
            ...
            case "remove" -> {
                if (parameters.length == 2) {
                    Kawaii.FRIEND.removeFriend(parameters[1]);
                    CommandManager.sendChatMessage("§f" + parameters[1] + (Kawaii.FRIEND.isFriend(parameters[1]) ? " §ahas been friended" : " §chas been unfriended"));
                    return;
                }
                sendUsage();
	...
	public String[] getAutocorrect(int count, List<String> seperated) {
		if (count == 1) {
			...
			for (String x : list) {
				if (input.equalsIgnoreCase(Kawaii.PREFIX + "friend") || x.toLowerCase().startsWith(input)) {
					correct.add(x);
				}
			}
