...
public class ToggleCommand extends Command {
	...
	public void runCommand(String[] parameters) {
		...
		Module module = Kawaii.MODULE.getModuleByName(moduleName);
		if (module == null) {
	...
	public String[] getAutocorrect(int count, List<String> seperated) {
		if (count == 1) {
			...
			for (Module x : Kawaii.MODULE.modules) {
				if (input.equalsIgnoreCase(Kawaii.PREFIX + "toggle") || x.getName().toLowerCase().startsWith(input)) {
