package dev.kizuna.mod.modules;
...
public abstract class Module extends Mod {
    ...
    public void sendNotify(String string) {
        if (HUD.INSTANCE.notification.getValue()) {
            NotificationHUD.notifyList.add(new NotificationUtil(string, this.getCategory()));
        }
    }
    ...
    public void enable() {
        ...
        if (!nullCheck() && drawnSetting.getValue() && ClientSetting.INSTANCE.toggle.getValue()) {
            ...
            if (HUD.INSTANCE.notification.getValue()) {
                sendNotify(this.getDisplayName() + " §a" + "Enabled");
            } else {
                sendTraditionalMessageOnEnable(id);
            }
        }
        this.state = true;
    ...
    public void disable() {
        ...
        if (!nullCheck() && drawnSetting.getValue() && ClientSetting.INSTANCE.toggle.getValue()) {
            int id = ClientSetting.INSTANCE.onlyOne.getValue() ? -1 : hashCode();
            if (HUD.INSTANCE.notification.getValue()) {
                sendNotify(this.getDisplayName() +  " §c" + "Disabled");
            } else {
                sendTraditionalMessageOnDisable(id);
            }
        }
        this.state = false;
        Kawaii.EVENT_BUS.unsubscribe(this);
        this.onToggle();
        this.onDisable();
    }

    private void sendTraditionalMessageOnEnable(int id) {
    ...
    private void sendTraditionalMessageOnDisable(int id) {
        switch (ClientSetting.INSTANCE.messageStyle.getValue()) {
