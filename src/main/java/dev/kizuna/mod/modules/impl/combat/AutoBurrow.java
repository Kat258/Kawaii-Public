...
import dev.kizuna.api.utils.combat.MeteorExplosionUtil;
...
public class AutoBurrow extends Module {
    ...
    private final SliderSetting Delay = add(new SliderSetting("Delay", 100, 0, 1000));
    ...
    private final SliderSetting cRange = add(new SliderSetting("Range", 5.0, 0.0, 6.0 , () -> ccheck.isOpen()));
    private final SliderSetting breakMinSelf = add(new SliderSetting("BreakSelf", 12.0, 0.0, 36.0, () -> ccheck.isOpen()));
    public BooleanSetting mcheck = add(new BooleanSetting("CheckMine", true).setParent());
    public BooleanSetting mself = add(new BooleanSetting("Self", true, () -> mcheck.isOpen()));
    private final SliderSetting predictTicks = add(new SliderSetting("PredictTicks", 4, 0, 10));
    private final BooleanSetting terrainIgnore = add(new BooleanSetting("TerrainIgnore", true));
    ...
    public boolean checkmine(boolean self) {
        ...
        for (BlockPos pos1 : pos) {
            if (pos1.equals(PacketMine.breakPos)) {
                return true;
            }
        }
        return false;
