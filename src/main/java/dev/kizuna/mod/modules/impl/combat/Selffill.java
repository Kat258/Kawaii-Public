...
import dev.kizuna.core.impl.BreakManager;
...
public class Selffill extends Module {
    ...
    private final BooleanSetting OnGround =
            add(new BooleanSetting("OnGround", true, () -> page.getValue() == Page.Misc));
    private final BooleanSetting antiLag =
            add(new BooleanSetting("AntiLag", true, () -> page.getValue() == Page.Misc));
    private final BooleanSetting noSelfPos =
            add(new BooleanSetting("NoSelfPos", false, () -> page.getValue() == Page.Misc));
    private final BooleanSetting breakCrystal =
            add(new BooleanSetting("BreakCrystal", true, () -> page.getValue() == Page.Misc));
    private final BooleanSetting wait =
            add(new BooleanSetting("Wait", true, () -> page.getValue() == Page.Misc));
    private final BooleanSetting headFill =
            add(new BooleanSetting("HeadFill", true, () -> page.getValue() == Page.Misc));
    //RotateBypass
    private final EnumSetting<RotateMode> rotate =
            add(new EnumSetting<>("RotateMode", RotateMode.Bypass , () -> page.getValue() == Page.RotateBypass));
    //Move
    private final BooleanSetting fakeMove =
            add(new BooleanSetting("FakeMove", true , () -> page.getValue() == Page.Move).setParent());
    private final BooleanSetting center =
            add(new BooleanSetting("AllowCenter", false, () -> page.getValue() == Page.Move && fakeMove.isOpen()));
    //Packet
    public final EnumSetting<Mode> Packet = add
            (new EnumSetting<>("LagMode", Mode.Au, () -> page.getValue() == Page.Packet));

    private final SliderSetting smartX =
            add(new SliderSetting("SmartXZ", 3, 0, 10, 0.1, () -> Packet.getValue() == Mode.Smart));

    private final SliderSetting smartUp =
            add(new SliderSetting("SmartUp", 3, 0, 10, 0.1, () ->   Packet.getValue() == Mode.Smart));

    private final SliderSetting smartDown =
            add(new SliderSetting("SmartDown", 3, 0, 10, 0.1, () -> Packet.getValue() == Mode.Smart));

    private final SliderSetting smartDistance =
            add(new SliderSetting("SmartDistance", 2, 0, 10, 0.1, () -> Packet.getValue() == Mode.Smart));



    private final SliderSetting off =
            add(new SliderSetting("INLagOff", 2, 0, 10, 0.1, () -> Packet.getValue() == Mode.Invalid));

    private final BooleanSetting Bypass =
            add(new BooleanSetting("INBypass", false, () -> Packet.getValue() == Mode.Invalid));

    private final SliderSetting offAu =
            add(new SliderSetting("AULagOff", 2, 0, 10, 0.1, () -> Packet.getValue() == Mode.Au));

    private final BooleanSetting BypassAu =
            add(new BooleanSetting("AUBypass", false, () -> Packet.getValue() == Mode.Au));

    private final SliderSetting offTo =
            add(new SliderSetting("VDLagOff", 108, 0, 109, 0.1, () -> Packet.getValue() == Mode.ToVoid));

    private final BooleanSetting BypassTo =
            add(new BooleanSetting("VDBypass", false, () -> Packet.getValue() == Mode.ToVoid));
    private final BooleanSetting self =
            add(new BooleanSetting("SelfLag", false, () -> Packet.getValue() == Mode.Nu));


    private final SliderSetting offAuto =
            add(new SliderSetting("LagOff", 1, -1, 2, 0.1, () -> Packet.getValue() == Mode.Auto));


    private final BooleanSetting ScannerY =
            add(new BooleanSetting("Scanner", false, () -> Packet.getValue() == Mode.Scanner));

    private final BooleanSetting tick =
            add(new BooleanSetting("TickLag", false, () -> Packet.getValue() == Mode.Scanner));

    private final SliderSetting AACOff =
            add(new SliderSetting("AACLagOff", 2, -10, 10, 0.1, () -> Packet.getValue() == Mode.Scanner));

    //FakeJump
    public final EnumSetting<FakeJumpMode> Jump = add
            (new EnumSetting<>("LagMode", FakeJumpMode.Normal, () -> page.getValue() == Page.FakeJump));

    //Place
    private final SliderSetting blocksPer =
            add(new SliderSetting("BlocksPer", 4, 1, 9, 1, () -> page.getValue() == Page.Place));

    private final BooleanSetting enderChest =
            add(new BooleanSetting("EnderChest", true, () -> page.getValue() == Page.Place));

    private final BooleanSetting inventory =
            add(new BooleanSetting("InventorySwap", true, () -> page.getValue() == Page.Place));
    public final BooleanSetting helper =
            add(new BooleanSetting("Helper", true, () -> page.getValue() == Page.Place));

    private final BooleanSetting packetPlace =
            add(new BooleanSetting("PacketPlace", true, () -> page.getValue() == Page.Place));

    private final BooleanSetting sound =
            add(new BooleanSetting("Sound", true, () -> page.getValue() == Page.Place));
    ...
    private void Misc(UpdateWalkingPlayerEvent event) {
        //Misc
        //先检测玩家isOnGround
    ...
    private void FakeJump(UpdateWalkingPlayerEvent event) {
        //NullPoint FakeJump
        switch (Jump.getValue()) {
            case NullPoint -> {
                ...
                if (this.rotate.getValue() == RotateMode.Bypass) {
                    event.cancelRotate();
                    EntityUtil.sendYawAndPitch(Kawaii.ROTATION.rotationYaw, 90);
                }
                if (!mc.player.isOnGround()) {
                    return;
                }
        ...
        switch (Packet.getValue()) {
            ...
            case Strict -> {
                ...
                for (i = 0; i < 10; ++i) {
                    ...
                    if (!this.canGoto(pos) || MathHelper.sqrt((float)((float) Selffill.mc.player.squaredDistanceTo(pos.toCenterPos()))) < 2.0f || bestPos != null && !(Selffill.mc.player.squaredDistanceTo(pos.toCenterPos()) < distance)) continue;
                    bestPos = pos;
                    distance = Selffill.mc.player.squaredDistanceTo(pos.toCenterPos());
                }
                if (bestPos == null) break;
                Selffill.mc.player.networkHandler.sendPacket((Packet)new PlayerMoveC2SPacket.PositionAndOnGround((double)bestPos.getX() + 0.5, (double)bestPos.getY(), (double)bestPos.getZ() + 0.5, false));
            }
    ...
    public boolean canPlace(BlockPos pos) {
        ...
        if(AutoBurrow.INSTANCE.isOn() && AutoBurrow.INSTANCE.mcheck.getValue()) {
            for (BreakManager.BreakData breakData : new HashMap<>(Kawaii.BREAK.breakMap).values()) {
    ...
    private void gotoPos(BlockPos offPos) {
        if (rotate.getValue() == RotateMode.None) {
            ...
            if (mc.player != null) {
                mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.2, offPos.getZ() + 0.5, Kawaii.ROTATION.rotationYaw, 90, false));
            }
        }
    }
