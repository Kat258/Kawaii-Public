...
import static dev.kizuna.api.utils.world.BlockUtil.hasCrystal;


public class ObiPlacer extends Module {
    public static ObiPlacer INSTANCE;
    ...
    public ObiPlacer() {
        super("ObiPlacer", Category.Combat);
        setChinese("自动水晶底座");
        INSTANCE = this;
    }
    ...
    @EventHandler
    public boolean onRotate(LookAtEvent event) {
    ...
    private void update() {
        ...
        if (breakOnlyHasCrystal.getValue() && !mc.player.getMainHandStack().getItem().equals(Items.END_CRYSTAL) && !mc.player.getOffHandStack().getItem().equals(Items.END_CRYSTAL) && !KawaiiAura.INSTANCE.findCrystal()) {
            tempPos = null;
            return;
        }
        delayTimer.reset();
