...
import dev.kizuna.core.impl.ShaderManager;
...
public class ShaderLatest extends Module {

    public static ShaderLatest INSTANCE;

    public ShaderLatest() {
        super("ShaderLatest", Category.Render);
        setChinese("着色器");
        INSTANCE = this;
    }
    private enum Page {
    ...
    public final EnumSetting<renderMode> RenderMode = add(new EnumSetting<>("Mode", renderMode.Both, () -> page.getValue() == Page.Shader && mode.is(ShaderManager.Shader.Solid)));
    public final SliderSetting speed = add(new SliderSetting("Speed", 4, 0, 20, 0.1, () -> page.getValue() == Page.Shader));
    public final ColorSetting fill = add(new ColorSetting("Color", new Color(0x6EACAFFD,true), () -> page.getValue() == Page.Shader));
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (hands.getValue())
            Kawaii.SHADER.renderShader(()-> mc.gameRenderer.renderHand(matrixStack, mc.gameRenderer.getCamera(), mc.getTickDelta()), mode.getValue());
    }

    @Override
    public void onToggle() {
        Kawaii.SHADER.reloadShaders();
    }
