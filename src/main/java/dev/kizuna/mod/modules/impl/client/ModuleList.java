...
import dev.kizuna.api.utils.math.AnimateUtil;
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        for (Modules modules : modulesList) {
            ...
            if (backGround.getValue()) {
                RenderShaderUtil.drawBlurredShadow(
                        drawContext.getMatrices(),
                        textX - 1 - 5,
                        (int) modules.y - 5,
                        ((float) mc.getWindow().getScaledWidth() - xOffset.getValueInt() + 1) - textX + 1 + 10,
                        getFontHeight() + height.getValueInt() + 10,
                        15,
                        new Color(16, 16, 16, 89)
                );
                Render2DUtil.drawRect(drawContext.getMatrices(), textX - 1,
                        ...
                        bgSync.getValue() ? ColorUtil.injectAlpha(Colors.INSTANCE.getColor(counter), (int) (bgColor.getValue().getAlpha() * modules.fade)) : ColorUtil.injectAlpha(bgColor.getValue().getRGB(), (int) (bgColor.getValue().getAlpha() * modules.fade)));
            }
            if (font.getValue()) {
                FontRenderers.ui.drawString(drawContext.getMatrices() ,getSuffix(modules.name), textX, (int) (modules.y + 1 + textOffset.getValueInt()), ColorUtil.injectAlpha(Colors.INSTANCE.getColor(counter), (int) (255 * modules.fade)));
            } else {
                drawContext.drawTextWithShadow(mc.textRenderer, getSuffix(modules.name), textX, (int) (modules.y + 1 + textOffset.getValueInt()), ColorUtil.injectAlpha(Colors.INSTANCE.getColor(counter), (int) (255 * modules.fade)));
            }
