...
import dev.kizuna.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], Kawaii.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - Kawaii.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(UpdateWalkingPlayerEvent event) {
        if (MoveFix.INSTANCE.isOn() && !MoveFix.INSTANCE.updateMode.is(MoveFix.UpdateMode.UpdateMouse)) {
            if (event.isPost()) {
                updateNext();
            }
        }
    ...
    public void update(MouseUpdateEvent event) {
        if (mc.player != null && MoveFix.INSTANCE.isOn() && !MoveFix.INSTANCE.updateMode.is(MoveFix.UpdateMode.MovementPacket)) {
            updateNext();
        }
    }
    private void updateNext() {
        RotateEvent rotateEvent = new RotateEvent(mc.player.getYaw(), mc.player.getPitch());
        Kawaii.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        Kawaii.EVENT_BUS.post(lookAtEvent);
